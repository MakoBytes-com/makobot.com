    ...
    <div className="min-h-screen">
      ...
      <section className="pt-8 pb-12 px-6">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-lg text-[#777777] max-w-2xl mx-auto mb-4">
          ...
          <div className="flex items-center justify-center gap-4 mb-8">
            ...
            {session?.user && (
              <Link
                ...
                className="inline-flex items-center px-6 py-3 rounded-lg bg-[#f8f9fb] hover:bg-[#e6f0f9] text-[#333333] font-semibold border border-[#dbdbdb] transition-colors"
              >
                My Listings
              </Link>
            )}
      ...
      {!search && !category && !platform && trending.length > 0 && (
        <div className="max-w-6xl mx-auto px-6 mb-6">
          <div className="border-t border-[#dbdbdb]/50" />
          <h2 className="text-lg font-bold text-[#333333] mt-6 mb-2">Browse All AI Skills, Prompts, and Configs</h2>
        </div>
      )}
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="bg-[#f8f9fb] rounded-xl p-5 border border-[#dbdbdb] animate-pulse"
                >
                  <div className="h-3 bg-[#dbdbdb] rounded w-20 mb-4" />
                  <div className="h-5 bg-[#dbdbdb] rounded w-3/4 mb-2" />
                  <div className="h-4 bg-[#dbdbdb] rounded w-full mb-1" />
                  <div className="h-4 bg-[#dbdbdb] rounded w-2/3 mb-4" />
                  <div className="flex gap-2">
                    <div className="h-5 bg-[#dbdbdb] rounded-full w-16" />
                    <div className="h-5 bg-[#dbdbdb] rounded-full w-14" />
                  </div>
                </div>
              ))}
            </div>
              ...
              {(page > 0 || hasMore) && (
                <div className="flex items-center justify-center gap-4 mt-10">
                  <button
                    ...
                    disabled={page === 0}
                    className="px-5 py-2.5 rounded-lg bg-[#f8f9fb] border border-[#dbdbdb] text-sm font-medium text-[#333333] hover:bg-[#e6f0f9] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                  >
                    Previous
                  </button>
