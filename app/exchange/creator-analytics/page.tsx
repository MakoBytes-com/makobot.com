      ...
      <div className="pt-8 pb-20 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-2 text-sm mb-6">
            ...
            <span className="text-[#333333] font-medium">Creator Analytics</span>
          </div>

          <h1 className="text-3xl font-bold text-[#333333] mb-2">Creator Analytics</h1>
          <p className="text-[#777777] mb-8">Track how your listings are performing on the exchange.</p>

          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="bg-[#f8f9fb] rounded-xl p-5 border border-[#dbdbdb] animate-pulse">
                  <div className="h-8 bg-[#dbdbdb] rounded w-20 mb-2" />
                  <div className="h-3 bg-[#dbdbdb] rounded w-16" />
                </div>
              ))}
            </div>
          ) : data ? (
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
                <div className="bg-[#f8f9fb] rounded-xl p-5 border border-[#dbdbdb]">
                  <p className="text-3xl font-bold text-[#0061aa]">{data.totals.listings}</p>
                  <p className="text-xs text-[#777777] mt-1">Listings</p>
                </div>
                <div className="bg-[#f8f9fb] rounded-xl p-5 border border-[#dbdbdb]">
                  <p className="text-3xl font-bold text-[#EC4899]">{data.totals.views.toLocaleString()}</p>
                  <p className="text-xs text-[#777777] mt-1">Total Views</p>
                </div>
                <div className="bg-[#f8f9fb] rounded-xl p-5 border border-[#dbdbdb]">
                  <p className="text-3xl font-bold text-[#10B981]">{data.totals.downloads.toLocaleString()}</p>
                  <p className="text-xs text-[#777777] mt-1">Downloads</p>
                </div>
                <div className="bg-[#f8f9fb] rounded-xl p-5 border border-[#dbdbdb]">
                  <p className="text-3xl font-bold text-[#F59E0B]">
                    {data.totals.views > 0 ? ((data.totals.downloads / data.totals.views) * 100).toFixed(1) : "0"}%
                  </p>
                  <p className="text-xs text-[#777777] mt-1">Conversion</p>
                </div>
                <div className="bg-[#f8f9fb] rounded-xl p-5 border border-[#dbdbdb]">
                  ...
                  <p className="text-xs text-[#777777] mt-1">{data.totals.reviews} Reviews</p>
                </div>
              </div>

              {/* Daily downloads chart */}
              <div className="bg-[#f8f9fb] rounded-xl p-6 border border-[#dbdbdb] mb-8">
                <h2 className="text-lg font-semibold text-[#333333] mb-4">Downloads (Last 30 Days)</h2>
                {data.downloadsPerDay.length > 0 ? (
                  <div className="flex items-end gap-1 h-40">
                    {data.downloadsPerDay.map((d, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        <div
                          className="w-full rounded-t bg-gradient-to-t from-[#0061aa] to-[#60A5FA] hover:opacity-80 transition-opacity"
                  ...
                  <p className="text-sm text-[#999999] text-center py-8">No downloads yet.</p>
                )}
              </div>

              {/* Top listings */}
              <div className="bg-[#f8f9fb] rounded-xl p-6 border border-[#dbdbdb]">
                <h2 className="text-lg font-semibold text-[#333333] mb-4">Your Top Listings</h2>
                {data.topListings.length > 0 ? (
                  <div className="space-y-3">
                    {data.topListings.map((l) => (
                      <div key={l.id} className="flex items-center justify-between py-3 border-b border-[#dbdbdb]/50 last:border-0">
                        <div className="flex-1 min-w-0">
                          <Link href={`/exchange/${l.slug}`} className="text-sm font-medium text-[#333333] hover:text-[#0061aa] truncate block">
                            {l.title}
                          </Link>
                        </div>
                        <div className="flex items-center gap-6 text-xs text-[#777777] shrink-0">
                  ...
                  <p className="text-sm text-[#999999] text-center py-8">You have no published listings yet.</p>
                )}
              </div>
            </>
          ) : (
            <p className="text-[#777777]">Failed to load analytics.</p>
          )}
        </div>
      </div>
      <Footer />
