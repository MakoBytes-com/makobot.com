...
MakoBot is a free Windows desktop application from Mako Logics LLC. It runs a Claude-powered assistant on the user's own computer. She lives in a chat window with tabs, has a face, talks and listens, and remembers. Her brain is Claude through the user's own Claude plan. Everything she stores stays on the user's disk. There is no Mako Logics server in the middle of any conversation, no subscription, and the app never shuts itself down.
...
- **Routines.** A spoken morning briefing on weekdays at 7:45, an end-of-day check at 5:30, and a Friday wrap-up. She suggests each once; nothing runs until accepted, a dismissed suggestion never returns, and a routine can only look, never act.
- **A second opinion.** One button brings another AI (GPT, Gemini, Grok, DeepSeek or Groq-hosted models, with the user's own key) into the chat as a critic. It sees the recent conversation and her answer, is told to lead with the most serious problem, has no tools, and cannot send or change anything. Only MakoBot acts, and only with approval. The user ticks a one-time consent before conversation context is shared.
...
- A few minutes after a chat goes quiet, a locked-down copy of her rereads it with memory tools only and saves at most two short notes.
- Roughly once a day she rereads her own notes for contradictions, repeated patterns and unanswered questions, and appends what she noticed.
...
- Every project folder the user adds gets MakoBot's memory tools in Claude Code automatically. She keeps the access keys in each project healthy so a session never silently loses its memory.
- Live Claude Code sessions are shown in the app as they start, both the user's own in VS Code and hers in the background.
- Her memory server runs on the local machine only, gated by a bearer token. Cursor and any other MCP client can search the memory, read the brain, and add a note.
...
- She will not fetch addresses inside the user's own network (routers, NAS, private ranges) even if a page or an email tries to steer her there.
...
- Every file she maintains (settings, routines, logs, memory indexes) detects corruption, sets the damaged copy aside, and rewrites itself fresh.
...
- Not a replacement for Claude Code, Cursor or the user's other AI tools. She sits above them.
