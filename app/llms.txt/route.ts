...
> MakoBot is a free Windows desktop application from Mako Logics LLC. It runs a Claude-powered assistant on the user's own machine. She watches the user's email across Outlook, Gmail, iCloud and Yahoo, tells genuine mail from forged mail using the sender's authentication records, keeps the user's Microsoft calendar and to-do list, listens through a local speech model so audio never leaves the machine, speaks with a natural voice, and can be reached from a phone over the user's own private Tailscale network. She remembers everything: one brain file across all of the user's work, a context file per project, every conversation saved in full, Claude Code sessions saved as transcripts, commits recorded as they happen, and ChatGPT, Claude.ai and Gemini chats captured by a browser extension. Memory is searchable by words or by meaning, entirely on the machine, backed up nightly, and can be synced encrypted across the user's own computers. She is the front door for Claude Code: every project the user adds gets her memory tools automatically, and any MCP client can use them. Every send, reply, forward and delete waits for the user's approval, and no setting can turn that off. Mailbox passwords are sealed with Windows data protection, secrets are scrubbed before anything is written, and updates run only when signed by Mako Logics LLC. The app never shuts itself down and runs a daily self-check that reports facts. Her brain runs on the user's own Claude plan. There is no subscription. Current release: version 3.0, build 393.

## Key URLs

- Home: https://makobot.com
- How she compares: https://makobot.com/compare
