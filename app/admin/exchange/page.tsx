...
export default function AdminExchangePage() {
  ...
  function renderListingCard(listing: ExchangeListing, showModeration: boolean) {
    ...
    return (
      <div key={listing.id} className="bg-[#f8f9fb] rounded-xl p-5 border border-[#dbdbdb]">
        {isEditing ? (
          /* ─── EDIT MODE ─── */
          <div className="space-y-4">
            <div>
              <label className="text-xs text-[#777777] mb-1 block">Title</label>
              <input value={editForm.title} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })}
                className="w-full px-3 py-2 rounded-lg bg-[#ffffff] border border-[#dbdbdb] text-[#333333] text-sm focus:outline-none focus:border-[#0061aa]" />
            </div>
            <div>
              <label className="text-xs text-[#777777] mb-1 block">Description</label>
              <textarea value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} rows={3}
                className="w-full px-3 py-2 rounded-lg bg-[#ffffff] border border-[#dbdbdb] text-[#333333] text-sm focus:outline-none focus:border-[#0061aa] resize-none" />
            </div>
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-xs text-[#777777] mb-1 block">Category</label>
              ...
              <div className="flex-1">
                <label className="text-xs text-[#777777] mb-1 block">Status</label>
            ...
            <div>
              <label className="text-xs text-[#777777] mb-1 block">Platforms</label>
              <div className="flex flex-wrap gap-2">
                {PLATFORMS.map((p) => (
                  <button key={p.value} type="button" onClick={() => toggleEditPlatform(p.value)}
                    className={`px-2 py-1 rounded-full text-xs font-semibold transition-colors ${editForm.platforms.includes(p.value) ? "text-white" : "bg-[#ffffff] text-[#777777] border border-[#dbdbdb]"}`}
            ...
            <div>
              <label className="text-xs text-[#777777] mb-1 block">Content</label>
              <textarea value={editForm.content} onChange={(e) => setEditForm({ ...editForm, content: e.target.value })} rows={6}
                className="w-full px-3 py-2 rounded-lg bg-[#ffffff] border border-[#dbdbdb] text-[#555555] text-sm font-mono focus:outline-none focus:border-[#0061aa] resize-none" />
            </div>
            <div className="flex gap-2">
              <button onClick={saveEdit} className="px-4 py-2 rounded-lg bg-[#0061aa] hover:bg-[#004d88] text-white text-sm font-semibold">Save</button>
              <button onClick={() => setEditingId(null)} className="px-4 py-2 rounded-lg bg-[#dbdbdb] hover:bg-[#777777] text-[#333333] text-sm">Cancel</button>
            </div>
          </div>
        ) : (
          /* ─── VIEW MODE ─── */
          <>
            <div className="flex items-start justify-between gap-4 mb-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  <h3 className="text-base font-semibold text-[#333333]">{listing.title}</h3>
                  <StatusBadge status={listing.status} />
                </div>
                <div className="flex flex-wrap items-center gap-3 text-xs text-[#999999]">
                  <span>{getCategoryLabel(listing.category)}</span>
                  <span>by <span className="text-[#0061aa]">@{listing.author_username || listing.author_name}</span> ({listing.author_email})</span>
            ...
            <p className="text-sm text-[#555555] mb-3 leading-relaxed">{listing.description}</p>

            {listing.content && (
              <div className="mb-3">
                <button onClick={() => setExpandedId(expandedId === listing.id ? null : listing.id)}
                  className="text-sm text-[#0061aa] hover:text-[#004d88] font-medium mb-2">
            ...
            {listing.file_name && (
              <p className="text-xs text-[#999999] mb-3">File: {listing.file_name} ({((listing.file_size || 0) / 1024).toFixed(1)}KB)</p>
            )}

            <div className="flex items-center gap-2 pt-3 border-t border-[#dbdbdb]/50">
                  ...
                  {rejectId === listing.id ? (
                    <div className="flex items-center gap-2 flex-1">
                      <input value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} placeholder="Reason (optional)"
                        className="flex-1 px-3 py-1.5 rounded-lg bg-[#ffffff] border border-[#dbdbdb] text-[#333333] text-xs focus:outline-none focus:border-[#DC2626]" />
                      ...
                      <button onClick={() => { setRejectId(null); setRejectReason(""); }}
                        className="text-xs text-[#777777] hover:text-[#333333]">Cancel</button>
                    </div>
                  ) : (
              ...
              <button onClick={() => startEdit(listing)}
                className="px-3 py-1.5 rounded-lg bg-[#0061aa]/10 hover:bg-[#0061aa]/20 text-[#0061aa] text-xs font-semibold">Edit</button>
              {listing.status !== "pending" && (
                <button onClick={() => handleModerate(listing.id, listing.status === "approved" ? "reject" : "approve")}
                  className="px-3 py-1.5 rounded-lg bg-[#dbdbdb] hover:bg-[#777777] text-[#333333] text-xs font-medium">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-[#333333]">Exchange Management</h1>
        <Link href="/admin/exchange/scrape" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#0061aa] hover:bg-[#004d88] text-white text-sm font-semibold transition-colors">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={s.label} className="bg-[#f8f9fb] rounded-xl p-4 border border-[#dbdbdb]">
            <p className="text-2xl font-bold text-[#0061aa]">{s.value}</p>
      ...
      <div className="flex gap-2 mb-6">
        {([
          ...
          <button key={t.value} onClick={() => setTab(t.value)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.value ? "bg-[#0061aa] text-white" : "bg-[#f8f9fb] text-[#777777] hover:text-[#333333] border border-[#dbdbdb]"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === "all" && (
        <div className="flex gap-3 mb-4">
          <input value={searchFilter} onChange={(e) => setSearchFilter(e.target.value)} placeholder="Search title, description, email..."
            className="flex-1 px-3 py-2 rounded-lg bg-[#f8f9fb] border border-[#dbdbdb] text-[#333333] text-sm placeholder-[#999999] focus:outline-none focus:border-[#0061aa]" />
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
            className="px-3 py-2 rounded-lg bg-[#f8f9fb] border border-[#dbdbdb] text-[#333333] text-sm focus:outline-none focus:border-[#0061aa]">
          ...
          <button onClick={() => loadTab("all")}
            className="px-4 py-2 rounded-lg bg-[#0061aa] hover:bg-[#004d88] text-white text-sm font-semibold">Filter</button>
        </div>
      )}
      ...
      {loading ? (
        ...
        pending.length > 0 ? (
          ...
          <div className="text-center py-12 bg-[#f8f9fb] rounded-xl border border-[#dbdbdb]">
            <p className="text-[#777777]">No pending listings.</p>
          </div>
        )
      ) : tab === "all" ? (
        allListings.length > 0 ? (
          ...
          <div className="text-center py-12 bg-[#f8f9fb] rounded-xl border border-[#dbdbdb]">
            <p className="text-[#777777]">No listings found.</p>
          </div>
        )
      ) : (
        /* Reviews tab */
        reviews.length > 0 ? (
          <div className="space-y-3">
            {reviews.map((review) => (
              <div key={review.id} className="bg-[#f8f9fb] rounded-lg p-4 border border-[#dbdbdb]">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    {review.reviewer_avatar && <img src={review.reviewer_avatar} alt="" className="w-6 h-6 rounded-full" />}
                    <span className="text-sm font-medium text-[#333333]">{review.reviewer_name}</span>
                    <span className="text-xs text-[#999999]">on</span>
                    <span className="text-sm text-[#0061aa]">{review.listing_title}</span>
                  </div>
                  <div className="flex items-center gap-3">
                ...
                {review.comment && <p className="text-sm text-[#777777]">{review.comment}</p>}
                <p className="text-xs text-[#777777] mt-1">{new Date(review.created_at).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        ) : (
          <div className="text-center py-12 bg-[#f8f9fb] rounded-xl border border-[#dbdbdb]">
            <p className="text-[#777777]">No reviews yet.</p>
          </div>
        )
      )}
    </div>
