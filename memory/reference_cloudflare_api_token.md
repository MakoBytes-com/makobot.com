---
name: Cloudflare API token — DNS edit on Russell's active zones
...
> ⚠️ **DEAD — verified invalid 2026-06-09** (`/user/tokens/verify` → "Invalid API Token";
...
**Dashboard:** https://dash.cloudflare.com/profile/api-tokens → find "Edit zone DNS" → choose **Roll** (keep scope, new value) or **Delete** (kill it entirely).

**Via API (would need another auth method):** `DELETE /user/tokens/b3e0f54bc93b3cfda720033378795185`.

## When to add more zones

If Russell asks to edit DNS on a zone not in the scope above, don't use this token on that zone — it'll return 403. Either:
1. Have Russell extend the existing token's zone list (same URL as dashboard above → edit the token)
...
- **TXT records >255 chars get split into multiple strings.** A DKIM value of ~400 chars shows up in the Cloudflare API as `"string1" "string2"` — valid DNS syntax. When comparing stored-vs-sent DKIM values, normalize by removing outer quotes and interior `" "` splits before diffing.
